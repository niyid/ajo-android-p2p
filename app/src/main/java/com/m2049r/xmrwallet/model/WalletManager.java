...
public class WalletManager {
    ...
    // NEW METHOD: Apply configuration from external config file path
    public void applyConfigurationFromFile(java.io.File configFile) {
        Properties props = new Properties();
        try (java.io.FileInputStream fis = new java.io.FileInputStream(configFile)) {
            props.load(fis);
            applyConfiguration(props);
            android.util.Log.d("WalletManager", "Loaded config from: " + configFile.getAbsolutePath());
        } catch (Exception e) {
            // Fall back to defaults if config file can't be read
            android.util.Log.w("WalletManager", "Failed to load config from: " + configFile.getAbsolutePath(), e);
        }
    }

    // NEW METHOD: Override wallet name for user-specific wallets
    public void setWalletNameOverride(String walletName) {
    ...
    public Wallet createWallet(String path, String walletPassword) {
        long walletHandle = createWalletJ(path, walletPassword, walletLanguage, toInt(networkType));
        return walletHandle != 0 ? new Wallet(walletHandle) : null;
    }
    

    public Wallet createWallet(String path, String walletPassword, String walletLanguage, int nettype) {
        long walletHandle = createWalletJ(path, walletPassword, walletLanguage, nettype);
        return walletHandle != 0 ? new Wallet(walletHandle) : null;
    }   
    ...
    public Wallet openWallet(String path, String password) {
        long walletHandle = openWalletJ(path, password, toInt(networkType));
        return walletHandle != 0 ? new Wallet(walletHandle) : null;
    }
